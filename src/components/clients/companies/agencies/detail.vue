          ...
          <v-row style="max-width: 100vw; margin:auto;">
            ...
            <v-col style="background:#f4f5fa;" class="py-6 px-4 mt-2" cols="12" md="9">
                <v-tabs grow class="paddingtab">
                  ...
                  <v-tab-item>
                    <sales v-bind:prop_agency_id="companyDetail.id"/>
                  </v-tab-item>
                  <v-tab background-color="transparent" color="basil" grow>
                    Cancelaciones
                  </v-tab>
                  <v-tab-item>
                    <canceled v-bind:prop_agency_id="companyDetail.id"/>
                  </v-tab-item>
                  <!--v-tab background-color="transparent" color="basil" grow>
                    Notas
                  </v-tab>
                  <v-tab-item>
                    <notes v-bind:prop_agency_id="companyDetail.id"/>
                  </v-tab-item>
                  ...
                  <v-tab-item v-show="permissions('activityLog')">
                    <activitylog v-bind:prop_agency_id="companyDetail.id"/>
                  </v-tab-item-->
                </v-tabs>
            </v-col>
          </v-row>
...
import ActivityLog from "../../../activitylog/container"
  export default {
    ...
    computed:{
      ...
      apiCall(){
        this.show = false
        axios.get(process.env.VUE_APP_BACKEND_ROUTE + "api/v1/agencies/" + this.$route.params.agency_id)
        .then(response=>{
