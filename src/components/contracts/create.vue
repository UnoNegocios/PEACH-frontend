                ...
                <v-row v-for="(item,k) in detail" :key="k" style="background-color: #94949417;" class="px-8 ma-0 pb-3 pt-0 mb-4">
                    ...
                    <v-row v-for="(item,j) in detail[k].items" :key="j" class="mb-2">
                        ...
                        <v-col ols="12" sm ="8" md="6" class="py-1 my-0">
                            <v-row>
                                <v-col cols="11">
                                    <v-autocomplete item-text="detail"  v-model="item.item" :disabled="detail[k].influencer_id==''" :items="itemLists(k)" item-value="name" label="Servicio" clearable>
                                        <template slot="no-data" class="pa-2">
                                            No existen servicios relacionados.
                                            <v-row>
                                                <v-col cols="6">
                                                    <v-text-field v-model="createService.name" label="Nuevo Servicio"></v-text-field>    
                                                </v-col>
                                                <v-col cols="4">
                                                    <v-text-field v-model="createService.price" label="Precio"></v-text-field>    
                                                </v-col>
                                                <v-col cols="2">
                                                    <v-btn icon class="pt-8" @click="saveNewService(detail[k].influencer_id)"><v-icon>mdi-plus</v-icon></v-btn>
                                                </v-col>
                                            </v-row>
                                        </template>  
