<template>    
    <v-card>
        <!-- Titulo del dialogo -->
        <v-card-title>
            Editar Influencer{{getDataFromApi}}
            <v-spacer></v-spacer>
                ...
                <v-stepper-items>
                    <v-stepper-content step="1" class="pt-0"-->
                        <v-row class="py-3 px-6"> 
                            ...
                            <v-col cols="12" sm="6" md="6" class="py-0">
                                <v-select v-model="editedItem.state" :items="statesList" item-value="id" item-text="name" label="Estado"></v-select>
                            </v-col>
                            <v-col cols="12" sm="6" md="6" class="py-0">
                                <v-select v-model="editedItem.city" :items="citiesList" item-value="id" item-text="name" label="Ciudad"></v-select>
                            </v-col>
                            <!--v-col cols="12" sm="6" md="6" class="py-0">
...
export default {
    ...
    data:()=>({ 
        bookingList:[{text:'Booking', value:1},{text:'Mgmt', value:0}],
        disable:'',
        citiesList:[],
        snackbar: {
            show: false,
    ...
    computed:{
        ...
        getDataFromApi(){
            axios.get(process.env.VUE_APP_BACKEND_ROUTE + "api/v1/cities?state_id=" + this.editedItem.state).then(response => {
    ...
    methods:{
        ...
        save () {
            this.gris = true
            this.$nextTick(() => {
                axios.patch(process.env.VUE_APP_BACKEND_ROUTE + "api/v1/influencers/" + this.editedItem.id,Object.assign(this.editedItem)).then(response=>{
                    this.snackbar = {
                        message: 'Influencer editado(a) con éxito!',
                        color: 'successful',
                        show: true
                    }
                    this.close()
