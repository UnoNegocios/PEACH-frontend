...
export default {
    ...
    mounted () {
        ...
        this.quotationsPast = this.$store.state.quotation.sales.filter(quotation=>new Date(quotation.updated_at) > new Date(this.startDateFromHistory)).filter(quotation=>new Date(quotation.updated_at) < new Date(this.startDateToHistory))
        this.render()
    },
    computed: {
