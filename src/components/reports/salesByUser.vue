<template>
    <v-container style="max-width:100vw;" class="mb-6">
        <v-row class="my-0 mx-4 px-12 pt-4" style="background:white;">
            <v-card-title>Ventas por Responsable</v-card-title>
            <v-spacer/>
        ...
        date_filter:{
            handler(){
                if(this.date_filter!=null && this.date_filter.length==2){
                    ...
                    axios.get(process.env.VUE_APP_BACKEND_ROUTE + 'api/v1/reports?subject=agents&include=agent&filter[deadline_date_between]='+this.date_filter).then(response=>{
